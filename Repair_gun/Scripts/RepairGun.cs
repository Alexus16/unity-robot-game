    ...
    private void Update()
    {
        ...
        //        }
            if (Targets.Count != 0)
            {
                ...
                foreach (GameObject i in Targets)
                {
                    AI temp = i.GetComponent<AI>();

                    if (Vector3.Distance(transform.position, i.transform.position) <= Vector3.Distance(target.transform.position, transform.position) && temp.HP!=temp.HP_MAX)
                    {
                        target = i;
